...
class HourlyForecast(BaseModel):
    ...
    def __str__(self):
        ...
        return f"At {local}, wave height: {self.wave_height:.1f} ft, period: {self.wave_period:.0f} s, direction: {self.wave_direction}°; " \
               f"wind wave height: {self.wind_wave_height:.1f} ft, period: {self.wind_wave_period:.0f} s, direction: {self.wind_wave_direction}°"

def forecast_response_to_hourly_entries(forecast: WaveForecastResponse) -> List[HourlyForecast]:
    entries = []
...
class WaveForecast(BaseModel):
    ...
    def within_time_range(self, cycle_start: datetime, cycle_end: datetime) -> List[HourlyForecast]:
        return WaveForecast(hourlies=[
            entry for entry in self.hourlies
